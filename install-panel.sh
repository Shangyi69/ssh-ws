...
def get_online_count(user):
    try:
        ...
        needle = f"sshd: {user}"
        # ပြင်ဆင်ချက် - Web UI တွင် [priv] ပါသော process အပိုများကြောင့် ၂ ဆမပေါ်စေရန် ဖယ်ထုတ်ရေတွက်ခြင်း
        return sum(1 for line in out.splitlines() if needle in line and "[priv]" not in line and "grep" not in line)
    except Exception:
        return 0
